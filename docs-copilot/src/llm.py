...
routes to Anthropic, OpenAI, or Ollama. This is the only module allowed to
call os.environ.get() on the API key secrets — copilot.py never touches
them directly, it just calls get_completion(). Unit tests mock
get_completion directly; it is never mocked in the main CLI code path.
"""

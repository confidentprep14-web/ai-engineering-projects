...
This module is packaged inside each variant's model.tar.gz (at the archive
root, alongside model.xgb) and is loaded by the SageMaker XGBoost framework
container via the SAGEMAKER_PROGRAM environment variable. SageMaker imports
these four functions and calls them in order for every real-time invocation:
...
Same structure as model-serving/src/inference.py (p3-07) -- both "current"
and "challenger" variants in this A/B test use this identical handler, since
the only difference between variants is the trained model weights, not the
serving code.
"""
import json
...
def model_fn(model_dir: str) -> xgb.XGBClassifier:
    """Load the model artifact written by prepare_variants.py's packaging step.
...
def input_fn(request_body, content_type: str) -> np.ndarray:
    ...
    Only text/csv is supported -- that's what invoke_traffic.py sends and
    what the endpoint is configured to accept.
    """

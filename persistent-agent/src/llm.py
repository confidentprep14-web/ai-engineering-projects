...
def get_completion(messages: list[dict], system: str = "") -> str:
    ...
    Takes the full conversation as a list of {"role", "content"} dicts plus a
    separate system prompt, so callers (fact extraction, the chat loop) share
    one code path regardless of which provider answers.
    """

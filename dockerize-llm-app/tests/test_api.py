...
This is the same application as p1-08-streaming-chat-api; these tests confirm
the app still behaves correctly with DATABASE_PATH pointed at a path outside
the container (the /data volume mount target), not just the old default."""
import os
import tempfile
...
def test_default_database_path_points_inside_data_volume():
    """The Docker-specific change from p1-08: DATABASE_PATH must default to
    somewhere inside the /data named volume, not the app directory, or data
    is lost on container restart."""
    from src.config import Config

    default_path = Config.__dataclass_fields__["database_path"].default
    assert default_path.startswith("/data/") or os.getenv("DATABASE_PATH")

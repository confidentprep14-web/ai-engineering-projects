...
def get_completion(prompt: str, system: str = "") -> str:
    """Send a single-turn completion request to the configured LLM provider.

    Each pipeline stage that needs the LLM sends one prompt and gets one
    response back — no conversation state is kept between stages.
    """
